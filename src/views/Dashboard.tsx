...
const Dashboard: React.FC = () => {
  ...
  const loadDashboardData = async () => {
    try {
      ...
      products.forEach(p => {
        ...
        catVolume[name] = (catVolume[name] || 0) + p.stock_actual;
      });

      const topCategories = Object.entries(catVolume)
        .map(([name, value]) => ({ name, value }))
        .sort((a, b) => b.value - a.value)
        .slice(0, 5);
  ...
  return (
    <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-700 pb-8">
      ...
      {/* Bottom Charts Grid */}
      <div className="grid grid-cols-2 gap-3 h-52 animate-in fade-in zoom-in-95 duration-500 delay-300">

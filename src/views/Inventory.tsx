  ...
  const handleEdit = (p: Producto) => {
    ...
    const handleEdit = (product: Producto) => {
      ...
      if (product.subcategoria_id) {
        const sub = categorias.find(c => c.id === product.subcategoria_id); // Changed 'categories' to 'categorias'
        if (sub && sub.parent_id) {
          setSelectedParentId(sub.parent_id);
        }
      }
      // Assuming `setActiveTab` and `loadHistory` are defined elsewhere in the component's scope
      // and `repository.getProductHistory` is available.
      // If not, these lines might cause errors.
      // For this change, I'm assuming they are part of the context.
      // @ts-ignore
      setActiveTab('details'); // Reset tab
      loadHistory(product.id); // Load history
